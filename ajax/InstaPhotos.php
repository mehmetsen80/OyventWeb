...
$processType= ($_GET['processType'])?$_GET['processType']:$_POST['processType'];
 ...
 switch($processType)
 {
 	case "GETUSERCONTENTLIST":
		getUserContentList();
		break;
	case "GETONLYUSERIMAGELIST":
		getOnlyUserImageList();
		break;
 }
  ...
  if(isset($userObject)){  	
	...
	if(isset($instadetails))
 	...
 	$call = new stdClass();
  	...
  	$call->pagination->next_url =   "https://api.instagram.com/v1/users/{$instadetails->user->id}/media/recent/?client_id={$clientID}&max_id={$maxID}";
 ...
 function getOnlyUserImageList(){
  ...
  $call->pagination = new stdClass();
  $call->pagination->next_max_id = $maxID;
  $call->pagination->next_url =   "https://api.instagram.com/v1/users/3/media/recent/?client_id={$clientID}&max_id={$maxID}";
